...
{
    [Script("ioc", "Dean")]
    public class Ioc : SpellScript
    {
        public Ioc(Spell spell) : base(spell)
        {
        }
        ...
        public override void OnUse(Sprite sprite, Sprite target)
        {
            if (target is Aisling && sprite is Aisling)
            {
                ...
                if (client.Aisling.CurrentMp >= Spell.Template.ManaCost)
                {
                    ...
                    target.Client.SendAnimation(109, target, client.Aisling);
                    client.Aisling.Show(Scope.NearbyAislings, action);

                    sprite.CurrentMp -= Spell.Template.ManaCost;
                    target.CurrentHp += (300 + ((Spell.Level + sprite.Wis) * 26));
                    ...
                    client.SendAnimation(105, target, sprite);
                ...
                if (sprite is Mundane)
                {
                    ...
                    if (nearby.Count() > 0)
                    {
                        foreach (var s in nearby)
                        {
                            ...
                            target.SendAnimation(109, target, target);
